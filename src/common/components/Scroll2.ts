...
export class ScrollComponent2 implements OnInit {
	constructor(
	...
	get isScrollDirectiveVisiable(): boolean {
		let target_top = $(this.elementRef.nativeElement).parent()[0].scrollHeight - 10;
		let view_top = 0;
		if ($(this.elementRef.nativeElement).parent().height()) {
			view_top = $(this.elementRef.nativeElement).parent().scrollTop() +
				$(this.elementRef.nativeElement).parent().height();
		} else {
